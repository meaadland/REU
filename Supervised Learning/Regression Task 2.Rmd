...
#0.937
lm.C5= lm(ListPrice~ poly(ImputedSQFT,3) + ImputedSQFT*City + ListPrice*City  + ParkSpots + Type*ImputedSQFT + HasGarage + BeenReduced + BATHS, data=ChiTrain)
summary(lm.C5)
plot(lm.C5)
anova(lm.C5)
```
...
```{r}
ypredlog=predict(lm.C5, newdata=ChiTest)
ypred = exp(ypredlog)
